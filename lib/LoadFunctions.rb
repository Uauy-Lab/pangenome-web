...
class LoadFunctions
  ...
  def self.insert_scaffolds_from_stream(stream,species, assembly, conn)
    ScaffoldHelper.insert_scaffolds_from_stream(stream, species, assembly, conn)
  end
...
def self.load_mutant_libraries(stream)
  ...
  def self.find_library(name, create:true)
    LineHelper.find_library(name, create: create)
  end


  def self.find_line(name)
    LineHelper.find_line(name)
  end
