...
class HaplotypeRegionSet extends RegionSet {
  ...
  calculateBaseAssembly(assembly) {
    ...
    if (assembly) {
      ...
      this.asm_blocks = this.color_blocks(
        longest["blocks"],
        i++,
        longest["region"].assembly
      );
  ...
  findAssemblyBlock(assembly) {
    var filtered_blocks = this.data.filter(
      (d) => d.merged_block == 0 && d.assembly == assembly
    );
  ...
  findLongestBlock() {
    ...
    return {
      region: longest,
      blocks: longest_arr,
      length: longest ? longest.length : 0,
    };
  }
