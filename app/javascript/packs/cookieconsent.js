...
class CookieConsentUtil {
  ...
  fillSelect(select, options, selected, cb) {
  ...
  getSimpleOption(label, value, selected) {
    return '<option '+(selected ? 'selected="selected"' : '')+' value="'+value+'">'+label+'</option>'
  }

  tabularObject(obj, formatVal, formatKey) {
  ...
  constructor(options) {
    ...
    //   console.log(CC);
    //   instances[ index ] = new CookieConsent( myOpts )
...
window.CookieConsentUtil = CookieConsentUtil;
