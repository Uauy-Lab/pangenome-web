...
class HaplotypeRegionPlot extends RegionPlot {
  ...
  event_coordinates(event) {
    ...
    var index =
      Math.round((this.status.coordinates.y + 0.5 * eachBand) / eachBand) - 1;
  ...
  blocks_hash(blocks, asm_index) {
    return (
      (asm_index + 1) ^
      blocks.sort().reduce((acc, curr, idx) => {
        let tmp = curr << idx % 32;
        return acc ^ tmp;
      }, 0)
    );
      ...
      .join(
        (enter) =>
          enter
            ...
            .attr(
              "width",
              (d) => self.previous_x(d.end) - self.previous_x(d.start)
            )
  ...
  updateFeatures(duration) {
    ...
    var self = this;
    this.svg_feature_rects
      .selectAll(".feature-in-plot")
  ...
  moveBars(update, duration, min_width) {
    return update
      ...
      .attr("width", (d) =>
        this.x(d.end) - this.x(d.start) < min_width
          ? min_width
          : this.x(d.end) - this.x(d.start)
      )
  ...
  updateBlocks(duration) {
    ...
    this.svg_main_rects
      ...
      .join(
        (enter) =>
          enter
            .append("rect") //.attr("class","block_rect")
            .attr("height", this.y.bandwidth())
            ...
            .attr(
              "width",
              (d) => this.previous_x(d.end) - this.previous_x(d.start)
            )
            .call((enter) => this.moveBars(enter, duration, 1)),
        (update) =>
          this.moveBars(update, duration, 1).style("opacity", (d) =>
            hb.length == 0 ? 1 : hb.includes(d.block_no) ? 1 : 0.1
          ),
